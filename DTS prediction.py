...
df_train = df_train.drop(columns=["DTC"])
df_train = df_train[~(df_train == -999).any(axis=1)]
df_train = df_train.sample(frac=1).reset_index(drop=True)
# Split the training data into features and labels
X_train = df_train.drop(columns=['DTS'])
y_train = df_train['DTS']
...
# Define the model
# Define the model
model = GradientBoostingRegressor(n_estimators=500, learning_rate=0.1, max_depth=6, random_state=0)
...
df_test['DTS'] = test_predictions
df_test = df_test[['DTS']]
df_test.to_csv('submission_oily_ML_DTS.csv', index=False)
